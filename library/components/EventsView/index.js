...
import { Card, CardBody, CardHeader, Checkbox, Dropdown, navigation, NerdGraphQuery, ngql as gql, Table, TableHeader, TableHeaderCell, TableRow, TableRowCell, Toast } from 'nr1';
...
const EventsView = ({ accountId, iDoc, selectedIDoc, eventsTimes, eventsTimeIndex, updateEventsTimeIndex }) => {
  const [events, setEvents] = useState([]);
  const [loading, setLoading] = useState(false);
  const [eventTypes, setEventTypes] = useState(EVENTS_TYPES.reduce((types, type) => ({...types, [type.type]: {type: type.display, selected: true}}), {}));
  
  useEffect(() => {
    if (!eventsTimes) return;
    const loadEvents = async() => {
      const [minTime, maxTime] = eventsTimes;
      const queryTime = maxTime > minTime ? `SINCE ${minTime} UNTIL ${maxTime}` : `SINCE ${minTime}`;
      ...
      const tracesQuery = `SELECT * FROM DistributedTraceSummary WHERE FUNCTION_NAME = '${iDoc.processFunction || ''}' ${queryTime} ${queryLimit}`;
      ...
      setLoading(true);
      const {loading, data: {actor: res} = {}, error} = await NerdGraphQuery.query({query, variables});
      setLoading(loading);
      console.log('events data', res)
      console.log('events error', error)
      const evts = Object.keys(res).reduce((evts, evt) => {
        if (evt === '__typename') return evts;
        const _ = (res[evt].results || []).map(e => {
          const event = {
            ...e,
            eventType: evt,
            eventColor: COLORS[evt === 'history' ? e.STATUS_LIGHT : evt.toUpperCase()],
          };
          if (evt === 'history') event.eventDescription = `${e.STATUS} ${e.STATUS_DESCRIPTION}. Message: ${e.STATUS_TEXT}`;
          if (evt === 'logs') event.eventDescription = e.message;
          if (evt === 'transports') event.eventDescription = `Transport: ${e.TRANSPORT} by ${e.OWNER} imported.  Return Code: ${e.RETURN_CODE}`;
          if (evt === 'rfcDestinations') event.eventDescription = `RFC Function: ${e.FUNCTION_NAME} returns status: ${e.RFC_STATUS} with message: ${e.RFC_MESSAGE}`;
          if (evt === 'traces') event.eventDescription = `Trace span of ${e.FUNCTION_NAME} at  ${e.DESTINATION} for ${e.DURATION} seconds. Called by ${e.PROGRAM_NAME} at line ${e.PROGRAM_LINE}`; // ${e['trace.id']}
          if (!evts.length) {
            evts.push(event);
          } else {
            let sortIdx = 0;
            while (sortIdx < evts.length && event.timestamp < evts[sortIdx]['timestamp']) sortIdx += 1;
  ...
  const toggleType = type => setEventTypes({...eventTypes, [type]: {type: eventTypes[type]['type'], selected: !eventTypes[type]['selected']}});

  const filteredEvents = events.filter(evt => eventTypes[evt.eventType]['selected']);
  ...
  const tableActions = [
    {
      label: 'Show details',
      onClick: (_, { item, index }) => {
        console.log('clicked', index, item)
        if (item.eventType === 'logs') return navigation.openStackedNerdlet({
  ...
  return (
    <div className="events-view">
      <Card>
        <CardHeader title="Events" subtitle={eventsTimes && eventsTimes.length === 2 ? `${formatHeaderTime(eventsTimes[0])} - ${formatHeaderTime(eventsTimes[1])}` : ''}>
          <Dropdown title={`${selectedEventsCount()} within ${EVENTS_TIMES[eventsTimeIndex]['display']}`}>
            <div className="events-filters">
              <div className="time-filters">
                {EVENTS_TIMES.map((time, i) => 
                  <Checkbox key={i} onChange={() => updateEventsTimeIndex(i)} label={time.display} checked={i === eventsTimeIndex} />
                )}
              </div>
              <div className="events-filters-divider" />
              <div className="type-filters">
                {Object.keys(eventTypes).map((type, i) => <Checkbox key={i} onChange={() => toggleType(type)} label={eventTypes[type]['type']} checked={eventTypes[type]['selected']} />)}
              </div>
            </div>
          </Dropdown>
        </CardHeader>
          ...
          <Table items={filteredEvents}>
            ...
            {({ item, index }) => (
              <TableRow key={index} actions={tableActions}>
                <TableRowCell style={{borderLeft: `5px solid ${item.eventColor}`}}>
