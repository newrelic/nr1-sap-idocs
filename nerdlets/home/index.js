...
const HomeNerdlet = () => {
  ...
  useEffect(() => {
    ...
    const query = gql`
      query IDocsQuery($accounts: [Int!]!, $nrqlQuery: Nrql!) {
        actor {
    ...
    const loadEntityProps = async () => {
      const propsToLoad =
        'latest(MESSAGE_TYPE) AS msgType, latest(SYS_ID) AS sId';
    ...
    const loadFilterLists = async (msgType, sId) => {
      const filterLists =
        'uniques(MESSAGE_TYPE) AS uniqMsgTypes, uniques(STATUS) AS uniqStatuses, uniques(SYS_ID) AS uniqSysIds';
      ...
      <div className="main-content">
        <div className="viz-section">
          ...
          <FilterBar
            ...
            updateMessageType={setMessageType}
            sysId={sysId}
            sysIds={sysIds}
